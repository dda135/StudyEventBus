...
public class EventBusAnnotationProcessor extends AbstractProcessor {
    ...
    private void collectSubscribers(Set<? extends TypeElement> annotations, RoundEnvironment env, Messager messager) {
        for (TypeElement annotation : annotations) {
            ...
            for (Element element : elements) {
                if (element instanceof ExecutableElement) {//查找那些将注解Subscribe用于method等可运行的元素
                    ExecutableElement method = (ExecutableElement) element;
                    if (checkHasNoErrors(method, messager)) {
                        TypeElement classElement = (TypeElement) method.getEnclosingElement();
                        methodsByClass.putElement(classElement, method);//记录当前注解的元素
                    }
                } else {
    ...
    private void createInfoIndexFile(String index) {
        ...
        try {
            ...
            writeIndexLines(writer, myPackage);//这里是直接做了静态的写入操作，将之前收集的结果写入SUBSCRIBER_INDEX这个map中
            writer.write("    }\n\n");
            writer.write("    private static void putIndex(SubscriberInfo info) {\n");
            writer.write("        SUBSCRIBER_INDEX.put(info.getSubscriberClass(), info);\n");
            writer.write("    }\n\n");
            writer.write("    @Override\n");
            writer.write("    public SubscriberInfo getSubscriberInfo(Class<?> subscriberClass) {\n");//后面EventBus通过这个方法从SUBSCRIBER_INDEX中获取即可
